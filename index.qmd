...
the CausalForestDML is better becuase of the plot showing its nonlinear.
...
leaves room for leftover residual variation. This leftover variation is what's
necessary for the causal model needs.
...
The results suggest that an increase in price is associated with a \~2.6%
decrease in sales.
...
plt.plot(income_values, dominicks_effect, label="Dominicks", c= "#8698F3")
plt.plot(income_values, minute_maid_effect, label="Minute Maid", c= "#A7CE5B")
plt.plot(income_values, tropicana_effect, label="Tropicana", c= "#FFBA7E")
...
The plot shows all three brand have negative elasticity so regardless of brand,
the model estimates there is a decrease in sales when price increases.
Dominick's seems to dip the lowest, and is possibly more sensitive to these
changes. To make the plot more readable I modified the income grid to 30 in
order to help smooth the jagged volatile plotted lines. The plot is noisy, which
leads me to believe the minute differences seen between the brands should not be
relied upon too heavily but rather the overall picture (such as the consensus on
negative elasticity).
...
-   CausalML Vignettes 1-3:
    ...
    -   3\) Quality Checks and Fit to Data: <https://youtu.be/ZxszowvvEoY>

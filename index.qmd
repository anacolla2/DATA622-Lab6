...
```{python}
#| title: "CausalForestDML Plot: Observations"
#| layout-ncol: 2
...
plt.scatter(true_cate_obs, cf_obs, alpha=0.3)
...
plt.scatter(true_cate_obs, linear_obs, alpha=0.3)
plt.axline((0, 0), slope=1)
...
The model explains a little more than half of the variation in log price which
leaves room for left-over residual variation.
...
cf_real= CausalForestDML(
  model_y=WeightedLassoCVWrapper(cv=3), model_t=WeightedLassoCVWrapper(cv=3), n_estimators=500, min_samples_leaf=10, max_depth=50, max_samples=0.45, random_state=646)

cf_real.fit(Y= Y_train,T= T_train,X= X_train,W= W_train)

#-- plot-- 
#-- income grid in original income units--
income_grid_raw= pd.DataFrame(
    np.linspace(X_train_raw["income"].min(), X_train_raw["income"].max(), 100),
    columns=["income"]
)
...
elasticity_grid= cf_real.effect(income_grid)
elasticity_lower, elasticity_upper= cf_real.effect_interval(income_grid)

#-- plot--
plt.figure(figsize=(10, 6))
plt.plot(income_grid_raw["income"], elasticity_grid, label="Estimated price elasticity")
plt.fill_between(income_grid_raw["income"],elasticity_lower,elasticity_upper,alpha=0.3,label="95% CI")
plt.plot(X_train_raw["income"],np.zeros_like(X_train_raw["income"]),"|",color="gray",alpha=0.1,markersize=5)
...
print(f"ATE 95% CI: ({ate_lower_real:.3f}, {ate_upper_real:.3f})")
```
...
```{python}
#-- R-score for CausalForestDML--
cf_rscore= cf_real.score(Y=Y_test,T=T_test,X=X_test,W=W_test)

#-- LinearDML fit--
linear_real= LinearDML(model_y=WeightedLassoCVWrapper(cv=3),model_t=WeightedLassoCVWrapper(cv=3),random_state=646)

linear_real.fit(Y=Y_train,T=T_train,X=X_train, W=W_train)

#-- R-score for LinearDML--
linear_rscore= linear_real.score(Y=Y_test,T=T_test,X=X_test,W=W_test)

print(f"CausalForestDML R-score: {cf_rscore:.4f}")
print(f"LinearDML R-score: {linear_rscore:.4f}")
print(f"Difference: {linear_rscore - cf_rscore:.4f}")
```
...
hypothetical confounder for the treatment and the target respectively. One
method for determining the range of `c_t` and `c_y` to explore by checking the
values of `c_t` and `c_y` for existing confounders. If you were to do this
...
`c_t=0.1` and `c_y=0.3`. Compute the sensitivity check for the most extreme
scenario, with `c_t=0.1` and `c_y=0.3`. What are the resulting confidence
intervals for the ATE? Do they contain 0?

```{python}
#-- sensitivity check for most extreme scenario--
sens_lower, sens_upper = cf_real.sensitivity_interval(
    c_t=0.1,
    c_y=0.3,
    alpha=0.05
)

print(f"Sensitivity interval: ({sens_lower:.3f}, {sens_upper:.3f})")

print(f"Does the sensitivity interval contain 0? {sens_lower <= 0 <= sens_upper}")
```

## Problem 4: CATE for Brands and Income

The three brands of orange juice have different price points and are targetted
at different customer segments, with `dominicks` as the discount brand,
...
the model. Calculate the feature importances for all the modifiers and plot the
elasticity as a function of income for each of the three brands. How do the
elasticities differ by brand?

```{python}
#-- brand dummy columns already created earlier--
...
#-- move brand from W into X--
prep_x_brand = oj[["income"] + brand_cols]
prep_w_brand = prep_w.drop(columns=brand_cols)

#-- split and scale-- 
Y_train_b, Y_test_b, T_train_b, T_test_b, X_train_raw_b, X_test_raw_b, W_train_raw_b, W_test_raw_b = ms.train_test_split(
    Y,
    T,
    prep_x_brand,
    prep_w_brand,
...
plt.barh(importance_df["modifier"], importance_df["importance"])
plt.xlabel("Feature importance")
plt.ylabel("Modifier")
plt.title("CATE Modifier Importances")
...
income_values = np.linspace(
    X_train_raw_b["income"].min(),
    X_train_raw_b["income"].max(),
    100
)

#-- brand scenarios--
dominicks_grid = pd.DataFrame({
    "income": income_values,
    "brand_minute.maid": 0,
    "brand_tropicana": 0
})

minute_maid_grid = pd.DataFrame({
    "income": income_values,
    "brand_minute.maid": 1,
    "brand_tropicana": 0
})

tropicana_grid = pd.DataFrame({
    "income": income_values,
    "brand_minute.maid": 0,
    "brand_tropicana": 1
})
...
plt.plot(income_values, dominicks_effect, label="Dominicks")
plt.plot(income_values, minute_maid_effect, label="Minute Maid")
plt.plot(income_values, tropicana_effect, label="Tropicana")
...
-   CausalML Vignettes 1-3:
    -   1\) Simulating Data: <https://youtu.be/UVcJWcCVq9g?si=7zLy2QUYuchql0yI>
